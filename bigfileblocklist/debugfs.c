...
/* zorro add */
	...
	struct ext3_extent_idx *ex;
	struct ext3_extent *ee;

	eh = (struct ext3_extent_header *)inode->i_block;
	ex = (struct ext3_extent_idx *)eh + 1;
	ee = (struct ext3_extent *)ex + 2;
	if (ex->ei_leaf > 1024 && !ext2fs_is_fast_symlink(inode)) {
		fprintf(out, "inode: %d extent_block: %d\n", inode_num, ex->ei_leaf);
	}
/***/
}
...
*/
	ext2_ino_t count;
	for (count=2;count<122101760;count++) {
		inode_buf = (struct ext2_inode *)
			malloc(EXT2_INODE_SIZE(current_fs->super));
		if (!inode_buf) {
			fprintf(stderr, "do_stat: can't allocate buffer\n");
		...
		if (debugfs_read_inode2(count, inode_buf, "stat",
					EXT2_INODE_SIZE(current_fs->super), 0)) {
			free(inode_buf);
			return;
		}
		dump_inode(count, inode_buf);
	}
	/*           */
	free(inode_buf);
	return;
